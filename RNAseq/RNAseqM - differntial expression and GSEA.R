...
# DE M vs F in young and old 
# "6.5weeks WT fasted male"   "15weeks WT fasted male"    "6.5weeks WT fasted female"  "15weeks WT fasted female"
con <- list(MALE = c(-1,1,0,0),
            FEMALE = c(0,0,-1,1),
            YOUNG = c(-1,0,1,0),
            OLD = c(0,-1,0,1),
            c(1,-1,-1,1))
...
for (i in 1:length(experimentGroups)){
  ...
  FCs2[[names(experimentGroups)[i]]] = de$fc
  pvals2[[names(experimentGroups)[i]]] = de$pval
  FDRs2[[names(experimentGroups)[i]]] = de$fdr
}
...
for (i in 21:length(files)){ #c(3,5,8,10)
...
ageSexTableFast <- merge(merge(FCs2$WT_FASTED[,c('WT_FASTED_', 'WT_FASTED_')], pvals2$WT_FASTED[,c('WT_FASTED_', 'WT_FASTED_')], by=0), FDRs2$WT_FASTED[,c('WT_FASTED_', 'WT_FASTED_')], by.x="Row.names", by.y=0)
row.names(ageSexTableFast) = ageSexTableFast$Row.names
ageSexTableFast = ageSexTableFast[,c(2,4,6)]
colnames(ageSexTableFast) = c('FC', 'pval', 'FDR')
 
aa = merge(ageSexTableFast, humanKfConversion, by.x=0, by.y='ncbi', all.x=T, sort=F)
aa$Finalsymbol = convertLoc2symbol(aa$Row.names)
write.csv(aa, paste0(path, 'outputFiles/interaction age sex fasted.csv'))


write.csv(FCwtVShet, paste0(path, 'outputFiles/G_FCrev.csv'))
write.csv(aa, paste0(path, 'outputFiles/G_FDRrev.csv'))
write.csv(pvalwtVShet, paste0(path, 'outputFiles/Gpval_rev.csv'))
